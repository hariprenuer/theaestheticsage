...
def twoPlanetsDistance(planet1, planet2, dt, sidereal=True):
	...
	p1_coord = swe.calc_ut(jt[1], p1_i, int(sidereal)*swe.FLG_SIDEREAL+swe.FLG_SPEED) #get position of planet1
	p1_x = p1_coord[0][0] #planet1's position (longitude/right ascension)
	p1_v = p1_coord[0][3] #planet1's speed in longitude/right ascension (deg/day)
	p2_coord = swe.calc_ut(jt[1], p2_i, int(sidereal)*swe.FLG_SIDEREAL+swe.FLG_SPEED) #get same values for planet2
	p2_x = p2_coord[0][0]
	p2_v = p2_coord[0][3]
	diff = p1_x - p2_x
...
def getNextIngress(planetName, start_dt, sidereal=True):
	...
	timedifftup = dt.timetuple()[:6] #year, month, day, hour, minute, second
	jt = swe.utc_to_jd(*timedifftup, swe.GREG_CAL) #convert to Julian time
	p_coord = swe.calc_ut(jt[1], p_i, int(sidereal)*swe.FLG_SIDEREAL+swe.FLG_SPEED) #get location info for planet
...
def printPlanetPositions(dt, orb, sidereal=True):
	#dt=datetime.datetime(1976,7,13,13,25,38,55)
	...
	nodepos=list(swe.nod_aps_ut(jt[1], i, int(sidereal)*swe.FLG_SIDEREAL+swe.FLG_SPEED) for i in range(len(NODES)))
	nodesigns=list(swe.split_deg(nodepos[i][0][0],8) for i in range(len(nodepos)))
	print(planetpos)
	...
	for i in range(len(NODES)):
		print(NODES[i]+':',str(nodesigns[i][0])+'°',str(nodesigns[i][1])+"'"+\
			str(nodesigns[i][2])+'"',SIGNKEY[nodesigns[i][4]],'   speed:',round(nodepos[i][0][3],3),'deg/day')
	perfectList = []
